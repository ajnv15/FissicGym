...
function Epoch(name,mode,targetelement,multiselect) {
	...
	*/
	function calConfig() {
		...
		self.minDate = new Date(2011,11,01);
		self.maxDate = new Date(2015,11,31);
	...
	*/
	...
	self.months_sh = new Array('Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre');
	self.monthup_title = 'Mes Siguiente';
		self.monthdn_title = 'Mes Anterior';
		self.clearbtn_caption = 'Borrar';
		self.clearbtn_title = 'Borrarlas fechas seleccionadas';
		self.maxrange_caption = 'Es el maximo de fechas posibles';
	...
	*/
	self.sendForm = function(form,inputname) {
		...
		for(var i=0;i<self.dates.length;i++) {
			...
			inp.setAttribute('value',encodeURIComponent(self.dates[i].dateFormat('d-m-Y')));  //default to the ISO date format
			f.appendChild(inp);
		}
		return true;
	};
